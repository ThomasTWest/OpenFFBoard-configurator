...
#include <Tle493d_conf.hpp>

extern "C" {
	#include "printf.h" //a C header, so wrap it in extern "C"
}
...
class Tle493d : public I2C_BitBang
{
  public:
	enum TypeAddress_e
	{
	...
	//Tle493d(AccessMode_e mode = FASTMODE, TypeAddress_e productType = TLE493D_A0);
	Tle493d(I2C_BitBang &port);
	...
	//void begin2(TwoWire &bus); //By TTW

	//void begin_w_addr(TwoWire &bus, uint8_t address); //By TTW
	 ...
	 */
	//void begin_long(TwoWire &bus, TypeAddress_e slaveAddress, bool reset, uint8_t oneByteRead);
  ...
  private:
	...
	bool readOut(uint8_t count);

	bool writeOut(uint8_t regAddr);
};
/*
#ifdef __cplusplus
}
